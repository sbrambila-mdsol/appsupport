use TPS_DBA
IF OBJECT_ID('dbo.uspSlackMessage') IS NOT NULL 
    BEGIN 
        DROP PROCEDURE dbo.uspSlackMessage
    END
go

CREATE PROCEDURE dbo.uspSlackMessage (@Message nvarchar(max), @Notify nvarchar(max) = '', @Message2 nvarchar(max) = '')
AS
BEGIN
  ...
  $cmd=hostname
  $Body_Text="'+@servername+': '+@Message+' at '+@timestamp+''+@Message2+''+@Notify+'"
   $SlackChannel=invoke-sqlcmd "select settingvalue from TPS_DBA.dbo.tblserversetting where settingname=''SlackCompletionChannel''"
  $SlackChannel=$SlackChannel.settingvalue -split ","

  foreach($channel in $SlackChannel){
  if ($SlackChannel -ne $null ){
  $SlackUser=invoke-sqlcmd "select settingvalue from TPS_DBA.dbo.tblserversetting where settingname=''SlackBotCompletionName''"
  $SlackURL=invoke-sqlcmd "select settingvalue from TPS_DBA.dbo.tblserversetting where settingname=''SlackURL''"
  $SlackIcon=invoke-sqlcmd "select settingvalue from TPS_DBA.dbo.tblserversetting where settingname=''SlackSuccessIcon''"
