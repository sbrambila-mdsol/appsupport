...
$global:LogFilePath = '.\OptimusProdLogs\AutoRunCreationLog.log'
...
try {
  ...
  Write-Log -Message "Created Directory for Logging"
		...
		$payload = @{
			"channel" = "#prod_bot"
			"icon_emoji" = ":autobot:" 
			"text" = "*Production Run Ticket Failed to be created on $localServer*"
...
set-JiraConfigServer 'https://trinitypharmasolutions.atlassian.net'
  try {
	...
	try {
    Write-Log -Message "Step 1: Select JIRA Ticket info from SQL"
$DeploymentDay = (Get-Date).DayOfWeek

  if ($DeploymentDay -eq 'Monday') { $tblRunTickets = Invoke-Sqlcmd -Query "SELECT DISTINCT * FROM TPS_DBA.dbo.tblRunTickets WHERE [Epic Link] like 'CSP-1' AND Active = 1" -ServerInstance localhost }
  if ($DeploymentDay -eq 'Tuesday') { $tblRunTickets = Invoke-Sqlcmd -Query "SELECT DISTINCT * FROM TPS_DBA.dbo.tblRunTickets WHERE [Epic Link] like 'CSP-2' AND Active = 1" -ServerInstance localhost }
  if ($DeploymentDay -eq 'Wednesday') { $tblRunTickets = Invoke-Sqlcmd -Query "SELECT DISTINCT * FROM TPS_DBA.dbo.tblRunTickets WHERE [Epic Link] like 'CSP-3' AND Active = 1" -ServerInstance localhost }
  if ($DeploymentDay -eq 'Thursday') { $tblRunTickets = Invoke-Sqlcmd -Query "SELECT DISTINCT * FROM TPS_DBA.dbo.tblRunTickets WHERE [Epic Link] like 'CSP-4' AND Active = 1" -ServerInstance localhost }
  if ($DeploymentDay -eq 'Friday') { $tblRunTickets = Invoke-Sqlcmd -Query "SELECT DISTINCT * FROM TPS_DBA.dbo.tblRunTickets WHERE [Epic Link] like 'CSP-5' AND Active = 1" -ServerInstance localhost }
  if ($DeploymentDay -eq 'Saturday') { $tblRunTickets = Invoke-Sqlcmd -Query "SELECT DISTINCT * FROM TPS_DBA.dbo.tblRunTickets WHERE [Epic Link] like 'CSP-12' AND Active = 1" -ServerInstance localhost }
  if ($DeploymentDay -eq 'Sunday') { $tblRunTickets = Invoke-Sqlcmd -Query "SELECT DISTINCT * FROM TPS_DBA.dbo.tblRunTickets WHERE [Epic Link] like 'CSP-13' AND Active = 1" -ServerInstance localhost }

Write-Log -Message "Step 2: Create JIRA Ticket"

$IssuesCreated=@()
foreach ($issue in $tblRunTickets){

$NewRep=Get-JiraUser -UserName $Issue.Assignee|Select-Object -ExpandProperty AccountId
...
Write-Log -Message "Step 3: Update Active JIRA Ticket"
Invoke-Sqlcmd -Query "EXEC TPS_DBA.dbo.uspSetServerSetting 'ActiveRunTicket', '$key'" -ServerInstance localhost 
}
...
If (-not ($key -eq $null) -or -not($key -eq ""))  {
		Write-Log -Message "'$key' JIRA Ticket Created Successfully"
	}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQ