...
#$IssuesCSV=import-csv c:\JQL\ProductionRunTicketsTest.csv
...
$cred = New-Object System.Management.Automation.PSCredential ("jirabot@shyftanalytics.com(opens in new tab)", $secpasswd)
set-JiraConfigServer 'https://trinitypharmasolutions.atlassian.net'
$Session=New-JiraSession -Credential $cred
$epichash = @{'Thursday' = 'CSP-4'; 'Friday' = 'CSP-5';'Monday' = 'CSP-1';'Tuesday'='CSP-2';'Wednesday'='CSP-3';'Saturday'='CSP-12';'Sunday'='CSP-13'}
$dte = (Get-Date).DayOfWeek
$CalcEpic = '$epichash.' + [string]$dte
$IssuesCreated=@()
#foreach ($issue in $IssuesCSV){
$parametersforjira=@{
Project= 'SHP'
IssueType='Production Run'
Summary='' + (Get-Date).DayOfWeek + ' Shire Daily Runs'
 Fields = @{
#  assignee = @{name='srajan@shyftanalytics.com(opens in new tab)'}
  customfield_10008=Invoke-Expression $CalcEpic
  customfield_11100 = @{value = "Yes"}
  customfield_13301 = @{value = "Yes"}
}
}
$IssueCreated=New-JiraIssue @parametersforjira -ErrorAction 'Stop'
 $Rep="srajan@shyftanalytics.com"
 $params=@{
...
$payload = @{
	"channel" = "#prod_bot"
    "icon_emoji" = ":autobot:" 
	"text" = "*Shire Production Run Ticket Have Been Created:*"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU1DiCiaPclE8X9qE2hriz2akB
# W+2gghtoMIIDtzCCAp+gAwIBAgIQDOfg5RfYRv6P5WD8G/AwOTANBgkqhkiG9w0B
...
# KwYBBAGCNwIBBDAjBgkqhkiG9w0BCQQxFgQU/t1l/ADXil/Y5TPWFZOu0liTbPUw
# DQYJKoZIhvcNAQEBBQAEggEAGcPKmQo5qDuyWMJtem0OlQBtHPzZWenZMcRewMTm
# vt4Z58Vi51/Mv4zHhWyB23Z4m+q4FHHbxZO+EiYTsjZfTuVhu09ZAlKdbLveXSH3
# /K8Fg+n56GSPMgCNOyQcEx9yRSwfl6BmqOBISed/Fw8st7MeXsn+sz0rANh3AkqJ
# aOrMwDNUG6mgbPu743EjKSbmHXZ5oEcM+QYbDuwwzGtprWhZ3IuYumeq3t/Ax0dh
# oPzTr4ZPbCkdz8y2kl36jE0EF1Ji7yNHkURn+Jt1ATvuaie0XSg5d3tWCK7kpF+i
# jGi6hKs/Fll6WNE++ygAXWttbRDS/M8pB9+5zUT29bX6+qGCAg8wggILBgkqhkiG
...
# MjAwMTE2MTU0NzQ5WjAjBgkqhkiG9w0BCQQxFgQUYeWkEd99P4T1r0XQY30UYENC
# nGcwDQYJKoZIhvcNAQEBBQAEggEAI//A1dp96gVUdnV2KwffH8sli36M6F4c7YSs
# d8tGVnZ4rDDwWyQ/VGb8dCX4qHufXbiMm2RlAHfS9sLHXxyohqBbfnUDAHCed6q1
# cZ2W93Ao6Z/MSEYRX7ZOD+b2MkzW8SxEzo303eWSlf5ZRiI/kRlHB1eathx4hZkB
# SSSeUlTwOCyn2U2iehQBHWMwW8/RejNs9Je0kjxKRHhyHOS1CFHrXpcyBtry6dws
# 6piuLhNdMP/zTgX3/MKnpg+68SG2Kz2lpatfJJkkYpi9iG9Vpm98ujcaTzrlUocC
# 7G+vT4PMpwPWofAeBh9rTp+hl9bRlt1LG81glJ/SWVY4hSGvpA==
