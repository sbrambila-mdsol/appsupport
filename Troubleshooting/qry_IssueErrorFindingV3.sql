...
DECLARE @@RunID nvarchar(500) =  NULL
		...
		BEGIN 
			...
			SET TPSRunID = CASE WHEN ' + isNULL(@@RunID, '''NULL''') + ' = ''NULL'' THEN 
				...
				,MaxFailTime = (
								...
								WHERE IsFailure = 1 
								AND QUERY <> ''SELECT COUNT(*) FROM AGD.vwLatestProcessLog where ErrorNumber>0''
								order by CONVERT(DATETIME, InsertDate) DESC)
								END
								ORDER BY CONVERT(DATETIME, insertdate) DESC
								)  
        ...
        '
         WITH ImportLogError (RowNumber, RunId, ImportID, ErrorMessage)
            ...
            ,
             ...
             ImportLog (RowNumber, RunID, ImportID, TpsScenarioTypeID, TPSDatafeedID, ImportSucceeded, RecordsLoaded, TaskQueueID)
            AS
            (
                SELECT
                    ROW_NUMBER() OVER(ORDER BY ImportedDate ASC) as Row#, a.RunID, a.ImportID, a.TpsScenarioTypeID, a.TPSDatafeedID, a.ImportSucceeded, a.RecordsLoaded, a.TaskQueueID
            ...
            SELECT RunID, ImportID, TPSScenarioTypeID, TPSDatafeedId, ErrorMessage, RecordsLoaded, TaskQueueID INTO ##ImportlogError FROM
            (
                select b.RunID, b.ImportID, b.TPSScenarioTypeID, b.TPSDatafeedID, a.ErrorMessage, b.RecordsLoaded, b.TaskQueueID from ImportLogError a
                join ImportLog b
                ...
                ON b.TPSDatafeedid = c.TPSdatafeedid
						...
						AND (c.IgnoreFileNotFound = 0 OR (c.IgnoreFileNotFound = 1 AND a.errormessage not like ''File Not Found%''))
            ) IMLog
        '
    EXEC (@sql)
...
IF (
	...
	BEGIN
		...
		b.importId, d.filedate, d.importtablename, d.TPSDatafeedID, b.RecordsLoaded, 
