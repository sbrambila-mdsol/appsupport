    ...
    SELECT (SELECT SettingValue FROM TPS_DBA.dbo.tblServerSetting WHERE SettingName LIKE ''''ClientName'''') AS Customer, ''''' + @DatabaseName + ''''' AS ''''Database'''', 	
	 QueryID = q.TPSQueryID
        ...
        from ' + @DatabaseName + '.agd.tblQAResults  ) AS Queries WHERE row_num = 2) qryP ON
				...
				INNER JOIN (Select TPSQueryID, TPSScenarioTypeId, avg_results = AVG(RESULT), STD_DEV = STDEV(RESULT), LOWER_BOUND = AVG(RESULT) - STDEV(RESULT), UPPER_BOUND = AVG(RESULT) + STDEV(RESULT) FROM ( SELECT DENSE_RANK() over (partition by TPSQueryID order by InsertDate desc) as row_num, * 
...
EXEC TPS_DBA.dbo.uspCreatevwCollatedQAResults
