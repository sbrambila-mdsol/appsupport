...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Refresh DCR Data', 
		...
		@command=N'UPDATE [VERASTEM].[AGD].[tblMdDataFeed]
SET ACTIVE = 0

UPDATE t
SET ACTIVE=1
FROM VERASTEM.agd.tblmddatafeed as t
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Stage Specialty DCR Push', 
		@step_id=2, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'exec uspProcessDCRSpecialty', 
		@database_name=N'VERASTEM', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
